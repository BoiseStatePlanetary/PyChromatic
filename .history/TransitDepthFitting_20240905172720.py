...
class fit_transit_depth():
    ...
    def build_param_object(self, use_exotic_data=False):
        if use_exotic_data:
            params = batman.TransitParams()
            params.t0 = self.ExoticResults["Tmid"]    #time of inferior conjunction (midtime)
            params.per = self.Priors["per"]           #orbital period
            params.rp = self.ExoticResults["Rp/R*"]   #planet radius (in units of stellar radii) - from EXOTIC FIT
            params.a = self.ExoticResults["a/R*"]     #semi-major axis (in units of stellar radii) - from EXOTIC FIT
            ...
            params.u = self.Priors["u"]               #limb darkening coefficient
    ...
    def fit_transit_depth(self):
        def calc_batman_curve(time, rp):
            params = self.BatmanParams  # starts with priors
            params.rp = rp
    ...
    def plot_transit(self, save=False):
        ...
        exotic_params = self.build_param_object(use_exotic_data=True)
        prior_curve = batman.TransitModel(exotic_params, self.PhotData["BJD_TDB"])
        prior_flux = prior_curve.light_curve(exotic_params)
        ...
        textstr = '\n'.join((f"Mid-transit time = {self.ExoticResults["Tmid"]:.4f}" + r" $\pm$ " + f"{self.ExoticResults["Tmid_unc"]:.6f}",
        ...
        if save: 
            if self.TelescopeName is not None:
                ...
                fig.savefig(f"{self.TransitDirectory}/{self.PlanetName}_{self.ObservationDate}_LC_FitRponly_priors.png", dpi=300, bbox_inches="tight")
