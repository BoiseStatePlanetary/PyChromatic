...
def plot_diff(PlanetName, ObservationDate, TransitDirectory, data_sets, standard, color, shift):
    ...
    for idx, data in enumerate(data_sets):
        ...
        plt.plot(new_time, y+shift[idx], color=color[idx], marker='.', ls='none', alpha=0.7, label=f"{data} - {standard}")
        plt.plot(np.arange(new_time[0], new_time-1, 25), np.repeat(shift, 25), color=color[idx], marker=None)
    plt.show()
...
if __name__ == "__main__":
    ...
    maintransit_dir = os.path.join(home_dir, "TrES-3b_Stacked_615", "AAVSO_reports_gray_calibration")
