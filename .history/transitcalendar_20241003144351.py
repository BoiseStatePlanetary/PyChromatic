...
def calculate_next_transits(start_time, dusk, dawn, number_of_transits, target, system):
    ...
    system : _type_
        _description_
    '''
    constraints = [AltitudeConstraint(min=20*u.deg), LocalTimeConstraint(min=dusk, max=dawn)]
...
def calculate_TrES3b_transits(start_time, number_of_transits):
    ...
    TrES3b = create_eclipsingsystem(primary_eclipse_time=Time(, format="jd"), 
                                    orbital_period=1.30618801690067*u.days, 
                                    eclipse_duration=1.4178*u.hours,
                                    target_name="TrES-3b")
...
def calculate_HATP23b_transits(start_time, number_of_transits):
    ...
    HATP23b = create_eclipsingsystem(primary_eclipse_time=Time(2459391.06350200, format="jd"), 
                                     orbital_period=1.30618801690067*u.days, 
                                     eclipse_duration=1.4178*u.hours, 
    ...
    dusk = dt.time(2, 0)  # 8pm at Boise State
    dawn = dt.time(10, 0)  # 4am at Boise State
    calculate_next_transits(start_time, dusk, dawn, number_of_transits, fixed_target, HATP23b)
