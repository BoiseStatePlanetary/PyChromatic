...
def calculate_next_transits(start_time, dusk, dawn, number_of_transits, target, system):
    ...
    dusk_utc = dusk + 6
    dusk_dt = dt.time(dusk_utc, 0)
    dawn_utc =  dawn + 6
    dawn_dt = dt.time(dawn_utc, 0)
    constraints = [AltitudeConstraint(min=30*u.deg), LocalTimeConstraint(min=dusk_dt, max=dawn_utc)]
...
def calculate_TrES3b_transits(start_time, number_of_transits):
    ...
    TrES3b = create_eclipsingsystem(primary_eclipse_time=Time(2454185.9104, format="jd"), 
                                    ...
                                    target_name="TrES-3b")
    dusk = 19  # 7pm at Boise State - LEAVE AS NUMBER, converted to UTC when transits are calculated.
