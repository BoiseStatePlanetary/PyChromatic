...
class fit_transit_depth():
    ...
    def load_phot_data(self):
        phot_data = np.loadtxt(os.path.join(self.TransitDirectory, f"AAVSO_{self.PlanetName}_{self.ObservationDate}.txt"), delimiter=",")
        return {"BJD_TDB" : phot_data[0], 
                "flux" : phot_data[1], 
                "error" : phot_data[2]}
    
    def parse_final_params(self, transit_dir, output_dir, ):
        json_name = f"FinalParams_{self.PlanetName}_{self.ObservationDate}"
        txt_name
