...
def load_transit_data(transit_dir, planet_name, date):
    ...
    green_data = np.loadtxt(os.path.join(transit_dir, "output", f"AAVSO_{planet_name}_{date}.txt"), delimiter=",")
...
def plot_data_with_curve(transit_dir, planet_name, date):
    ...
    green_params = parse_final_params(transit_dir, "output", f"FinalParams_{planet_name}_{date}.json", f"AAVSO_{planet_name}_{date}.txt")
...
def plot_RGB_vals(transit_dir, planet_name, date):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(transit_dir, f"RGB-vs-Midtime_{planet_name}_{date}.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(transit_dir, f"RGB-vs-Depth_{planet_name}_{date}.png"), dpi=300, bbox_inches='tight')
    plt.close()

if __name__ == "__main__":
    transit_dir = os.path.join(home_dir, "TessCandidate")
    planet_name = "TIC88996391.01"
    date = "7-31-2024"
