...
def plot_diff(PlanetName, ObservationDate, TransitDirectory, data_set, standard, color, shift):
    #load data from the observation that data will be subtracted from. 
    standard = load_phot_data(PlanetName, ObservationDate, TransitDirectory, data_set)
    # calculate JD
    JD = int(photdata["BJD_TDB"][0])
    time = photdata["BJD_TDB"] - JD
    plt.errorbar(time, photdata["flux"]+shift, yerr=photdata["error"], color=color, marker='.', ls='none', alpha=0.7, label=data_set)
