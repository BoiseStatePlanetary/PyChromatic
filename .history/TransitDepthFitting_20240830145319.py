...
def calc_curve(planet_params, time, rp):
...
def fit_transit_depth(phot_dat, planet_params):
    LM_popt, LM_pcov = curve_fit(calc_curve, phot_data["BJD_TDB"], phot_data["flux"], sigma=phot_data["error"], 
                             p0=[planet_params[planet]["rp"], planet_params[planet]["a"], planet_params[planet]["inc"]])
