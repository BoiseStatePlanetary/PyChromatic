...
# Use all priors and allow fit to only adjust the transit depth, no limb darkening
def fit_transit_depth(planet_params, time, rp):
...
def plot_transit(phot_data):
    fig = plt.figure(figsize=(10*aspect_ratio, 10))
    ax = fig.add_subplot(111)

    ax.errorbar(phot_data["BJD_TDB"], phot_data["flux"], yerr=phot_data[::2], marker='.', ls='', color=BoiseState_blue, alpha=0.5)
