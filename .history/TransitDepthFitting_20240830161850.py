...
class fit_transit_depth():
    ...
    def BatmanParams(self):
        """creates a batman params object for use throughout the class.. pulls values from priors in EXOTIC output"""
    ...
    def calc_batman_curve_rp(self, rp):
        self.BatmanParams.rp = rp
        m = batman.TransitModel(self.BatmanParams, self.PhotData["BJD_TBD"])    #initializes model
        flux = m.light_curve(self.BatmanParams) 
        return flux

    def fit_transit_depth(self):
        LM_popt, LM_pcov = curve_fit(self.calc_batman_curve, self.PhotData["BJD_TDB"], self.PhotData["flux"], sigma=self.PhotData["error"], 
                                    p0=self.Priors["rp"])
