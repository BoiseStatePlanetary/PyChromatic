...
class fit_transit_depth():
    def __init__(self, transit_directory=None, exotic_output_directory=None, planet_name=None, observation_date=None):
        # default colors 
    ...
    def load_phot_data(self):
        ...
        data_directory = os.path.join(self.TransitDirectory, self.)
        phot_data = np.loadtxt(os.path.join(self.TransitDirectory, txt_name), delimiter=",")
        return {"BJD_TDB" : phot_data[0], 
                "flux" : phot_data[1], 
                "error" : phot_data[2]}
    
    def parse_final_params(self, transit_dir, output_dir, ):
