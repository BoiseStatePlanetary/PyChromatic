...
def plot_data_with_curve(PlanetName, ObservationDate, scope_name, TransitDirectory, ExoticOutputDirectory, y_val, color):
...
def plot_multiple_curves(PlanetName, ObservationDate, TransitDirectory, compile_list):
    ...
    for color, data_set in enumerate(compile_list):
        y_val -= 0.23
        plot_data_with_curve(PlanetName, ObservationDate, os.path.join(TransitDirectory, data_set), "output_green", y_val, colors[color])
