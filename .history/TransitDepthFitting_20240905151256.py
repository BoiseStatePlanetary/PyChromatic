    ...
    def plot_transit(self, save=False):
        ...
        if save: 
            if self.TelescopeName is not None:
                fig.savefig(f"{self.TransitDirectory}/figures/{self.TelescopeName}_{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png", dpi=300, bbox_inches="tight")
                print(f"Figure saved to figures/{self.TelescopeName}_{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png")
            else:
                fig.savefig(f"figures/{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png", dpi=300, bbox_inches="tight")
                print(f"Figure saved to figures/{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png")
        else:
            fig.show()
