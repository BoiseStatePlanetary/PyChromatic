...
def plot_diff(PlanetName, ObservationDate, TransitDirectory, data_sets, standard, color, shift):
    ...
    for idx, data in data_sets:
        ...
        plt.plot(new_time, y+shift, color=color, marker='.', ls='none', alpha=0.7, label=f"{data} - {standard}")
        plt.plot(np.arange(new_time[0], new_time-1, 25), np.repeat(shift, 25), color=color, marker=None)
    plt.show()
...
if __name__ == "__main__":
    ...
    plot_diff(planet_name, date, maintransit_dir, ["d_f_df", "d_f"], "d", )
