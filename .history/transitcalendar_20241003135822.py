...
def calculate_next_transits(start_time, dusk, dawn, number_of_transits, target, system):
    ...
    system : _type_
        _description_
    '''
    ## NOTE: LocalTimeConstraint DOES NOT USE LOCAL TIME. min & max times must be in UTC for it to be functonal..
    dusk_utc = dusk + 6
    dawn_utc =  dawn + 6
    constraints = [AltitudeConstraint(min=30*u.deg), LocalTimeConstraint(min=dusk_utc, max=dawn_utc)]
    ...
    ingress_mdt = ingress_times_iso.to_datetime(timezone = mdt)
    egress_mdt = egress_times_iso.to_datetime(timezone=mdt)
    # print all transits
    for i, (ingress, egress) in enumerate(np.column_stack((ingress_times_iso, egress_times_iso))):
        ...
        print(f"Mountain Time: Ingress - {ingress_mdt[i]}, Egress - {egress_mdt[i]}")
...
def calculate_TrES3b_transits(start_time, number_of_transits):
    ...
    TrES3b = create_eclipsingsystem(primary_eclipse_time=Time(2454185.9104, format="jd"), 
                                    ...
                                    target_name="TrES-3b")
    dusk = dt.time(19, 0)  # 7pm at Boise State - LEAVE AS NUMBER, converted to UTC when transits are calculated.
