...
class fit_transit_depth():
    ...
    def plot_transit(self, save=False):
        ...
        # adjust x-axis 
        time = self.PhotData["BJD_TDB"] - self.PhotData["BJD_TDB"][0]
        ...
        ax.set_xlabel("Time - %s" % (np.min(self.PhotData["BJD_TDB"][0]), fontsize=36)
        ...
        textstr = '\n'.join((f"Mid-transit time = {self.Priors["Tmid"]:.4f}" + r" $\pm$ " + f"{self.Priors["Tmid_unc"]:.4f}",
