...
def plot_data_with_curve(PlanetName, ObservationDate, TransitDirectory, ExoticOutputDirectory, y_val):
    ...
    plt.text(photdata["flux"][-1,0]+0.0125, y_val, red_str, fontsize=12, color="r")
    # plot RED data & curve
    plt.errorbar(red_flux[:, 0], red_flux[:, 1]+0.1, yerr=red_flux[:, 2], color="r", marker='.', ls='none', alpha=0.7)
    plt.plot(red_flux[:, 0], red_model, lw=3.0, color="r")
...
def plot_multiple_curves(PlanetName, ObservationDate, compile_list):
    y_val = 1.1
    for i, data_set in enumerate(compile_list):
        y_val -= i
        plot_data_with_curve(PlanetName, ObservationDate, data_set["transit_dir"], data_set["exotic_dir"], y_val)
...
if __name__ == "__main__":
    ...
    transit_dir = os.path.join(home_dir, "TrES-3b_85")
