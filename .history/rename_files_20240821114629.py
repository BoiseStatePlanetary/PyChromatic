    ...
    for file in os.listdir(dir):
        if os.path.join(dir, file):
            if os.path.splitext(file)[1] == ".fits":
                my_files.append((file, os.path.join()))
    all_files = [file for file in os.listdir(dir) if os.path.isfile(os.path.join(dir, file))]
    print(all_files)
    fits_files = [(file, os.path.join(dir, file)) for file in os.listdir(dir) if os.path.isfile(os.path.join(dir, file))]
    fits_mask = [os.path.splitext(file) for (file, fullpath) in my_files]
    print()
