...
def rename_files():
    ...
    # use at least 4 digits, even if that's not actually required to uniquely assign values
    number_of_digits = [4 if len(number_of_digits) < 4]
    ...
    for index, (file, fullpath) in enumerate(sorted_by_creation_date):
        ...
        new_filename = f"{index + 1:04d}_{date}{telescope_name}_{planet_name}_{file_split[-1]}" 
