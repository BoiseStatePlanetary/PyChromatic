...
def calculate_next_transits(start_time, dusk, dawn, number_of_transits, target, system):
    ...
    filtered_transits = [time for time, is_observable in zip(ing_egr, observable_bool[0]) if is_observable]
    counter = 1
...
def calculate_TrES3b_transits():

    fixed_target = create_target(RA=268.0291, DEC=37.54633, target_name="TrES-3b")
    TrES3b = create_eclipsingsystem(primary_eclipse_time=Time(2454185.9104) , 
                                    orbital_period= , 
                                    eclipse_duration= , 
                                    target_name="TrES-3b")
