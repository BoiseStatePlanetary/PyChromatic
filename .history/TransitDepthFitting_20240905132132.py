...
class fit_transit_depth():
    ...
    def load_phot_data(self):
        phot_data = np.loadtxt(os.path.join(transit_dir, "output_red_10min", f"AAVSO_{planet_name}_{date}.txt"), delimiter=",")
        green_data = np.loadtxt(os.path.join(transit_dir, "output_green_10min", f"AAVSO_{planet_name}_{date}.txt"), delimiter=",")
        blue_data = np.loadtxt(os.path.join(transit_dir, "output_blue_10min", f"AAVSO_{planet_name}_{date}.txt"), delimiter=",")
        phot_dat = {"red_data"}
    ...
    def plot_transit(self, save=False):
        ...
        if save: 
            fig.savefig(f"figures/{self.TelescopeName}_{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png", dpi=300, bbox_inches="tight")
            print(f"Figure saved to figures/{self.TelescopeName}_{self.PlanetName}_{self.ObservationDate}_LC_FitRponly.png")
        else:
            fig.show()
